...
// DATA
import { UI_USECASE,TITLEINFO } from "../../utils/data/constant.ts";
...
const Home: React.FC = () => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setFade(false);
            setTimeout(() => {
                setIndex((prevIndex) => (prevIndex + 1) % TITLEINFO.length);
                setFade(true);
            }, 500);
        }, 1000);
                    ...
                    <div className="flex flex-col md:grid md:grid-cols-2 md:gap-4 items-center space-y-2">
                        <div className="flex flex-col justify-center md:items-start items-center">
                            ...
                            <div className="md:mt-6 mt-4 text-center md:text-start">
                                ...
                                <span className={`block text-blue-600 transition-opacity duration-500 ${fade ? 'opacity-100' : 'opacity-0'}`}>
                                    {TITLEINFO[index]}
                                </span>
                                </h1>
                            </div>
                            <div className="mt-6">
                                <p className="text-sm font-normal text-gray-600">A collection of the best React templates and tools curated and verified by
